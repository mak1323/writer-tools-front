...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(function () {
	  ...
	  $('#editDeleteModal').on('show.bs.modal', function (event) {
	    ...
	    var changer = $(event.relatedTarget); // Button that triggered
	    // gets the id from the array
	    var id = parseInt(changer[0].id);
	...
	var setAPIOrigin = function setAPIOrigin(location, config) {
	  ...
	  if (search.environment === 'development' || location.hostname === 'localhost' && search.environment !== 'production') {
	    if (!(config.apiOrigin = config.apiOrigins.development)) {
	      ...
	      config.apiOrigin = 'https://pure-tundra-23985.herokuapp.com/';
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var getFavoriteNames = function getFavoriteNames(data) {
	  api.getFavorites(data).then(ui.getFavoritesSuccess).catch(ui.getFavoritesFailure);
	};

	var getUpdateNameList = function getUpdateNameList(data) {
	  api.getFavorites(data).then(ui.getUpdateNameListSuccess).catch(ui.getUpdateNameListFailure);
	};

	var onUpdate = function onUpdate(event, data) {
	  event.preventDefault();
	  var id = data.favorite.id;
	  api.updateFavorite(data, id).then(ui.updateFavoritesSuccess).then(getUpdateNameList).catch(ui.updateFavoritesFailure);
	  getFavoriteNames(event);
	};

	var populateNouns = function populateNouns(event) {
	  event.preventDefault();
	  api.getNouns().then(ui.getNounsSuccess).catch(ui.getNounsFailure);
	};

	var populateAdjectives = function populateAdjectives(event) {
	  event.preventDefault();
	  api.getAdjectives().then(ui.getAdjectivesSuccess).catch(ui.getAdjectivesFailure);
	};

	var onSignIn = function onSignIn(event) {
	  event.preventDefault();
	  var data = getFormFields(this);
	  api.signIn(data).then(ui.signInSuccess).then(getFavoriteNames).catch(ui.signInFailure);
	  populateNouns(event);
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  if (data.credentials.password === data.credentials.password_confirmation) {
	    api.signUp(data).then(ui.signUpSuccess).then(function (creds) {
	      return onSignIn(creds);
	...
	var onCreateFavorite = function onCreateFavorite(event, data) {
	  event.preventDefault();
	  api.createFavorite(data).then(ui.updateFavoritesSuccess).then(getUpdateNameList).catch(ui.updateFavoritesFailure);
	};

	var onDestroyFavorite = function onDestroyFavorite(event, data, id) {
	  event.preventDefault();
	  api.destroyFavorite(data, id).then(ui.destroyFavoriteSuccess).then(getUpdateNameList).catch(ui.destroyFavoriteFailure);
	};
	...
	module.exports = {
	  ...
	  onUpdate: onUpdate,
	  populateNouns: populateNouns,
	  populateAdjectives: populateAdjectives,
	  onCreateFavorite: onCreateFavorite,
	  onDestroyFavorite: onDestroyFavorite,
	  getUpdateNameList: getUpdateNameList
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var getFavorites = function getFavorites(token) {
	  return $.ajax({
	    method: 'GET',
	...
	var getUpdateFavorites = function getUpdateFavorites(token) {
	  return $.ajax({
	    method: 'GET',
	    url: config.apiOrigin + 'favorites',
	    headers: {
	...
	// this will be a get request for names
	var getNouns = function getNouns(data) {
	  return $.ajax({
	    method: 'GET',
	...
	// this will be a get request for names
	var getAdjectives = function getAdjectives(data) {
	  return $.ajax({
	    method: 'GET',
	...
	var updateFavorite = function updateFavorite(data, id) {
	  return $.ajax({
	    method: 'PATCH',
	...
	// this is working perfectly fine
	var destroyFavorite = function destroyFavorite(data, id) {
	  return $.ajax({
	    method: 'DELETE',
	    url: config.apiOrigin + 'favorites/' + id,
	    headers: {
	...
	module.exports = {
	  ...
	  getUpdateFavorites: getUpdateFavorites
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(10);
	var showFavoritesTemplate = __webpack_require__(12);

	// global declerations
	var favorites = void 0;
	var nouns = void 0;
	var adjectives = void 0;
	...
	var resetModalValues = function resetModalValues() {
	...
	var clearFavorites = function clearFavorites(array) {
	  $('#saved-names').empty();
	  array = [];
	};

	// iterates through favorites array to find local favorites
	var favoritesDisplay = function favoritesDisplay(array) {
	...
	var getFavoritesSuccess = function getFavoritesSuccess(data) {
	  clearFavorites(store.favorites);
	  // stores favorites in store
	  store.favorites = data.favorites;
	  favorites = store.favorites;
	  var userFavorites = favoritesDisplay(favorites);
	  // append them with handlebars
	  var showFavoritesHTML = showFavoritesTemplate({ userFavorites: userFavorites });
	...
	var getFavoritesFailure = function getFavoritesFailure() {
	  message('no go');
	};

	// gets nouns and loads them from the store
	var getNounsSuccess = function getNounsSuccess(data) {
	  store.nouns = data.nouns;
	  nouns = store.nouns;
	};

	// get nouns failure
	var getNounsFailure = function getNounsFailure() {
	  message('nope');
	};

	// get adjectives and loads it in store
	var getAdjectivesSuccess = function getAdjectivesSuccess(data) {
	  store.adjectives = data.adjectives;
	  adjectives = store.adjectives;
	};

	// get adjectives failure.
	var getAdjectivesFailure = function getAdjectivesFailure() {
	  message('nope');
	};
	...
	var createFavoritesSuccess = function createFavoritesSuccess(data) {
	...
	// self explanitory
	var createFavoritesFailure = function createFavoritesFailure() {
	...
	// resets modals after.
	// returns a token for continuing a promise
	var updateFavoritesSuccess = function updateFavoritesSuccess() {
	...
	var updateFavoritesFailure = function updateFavoritesFailure() {
	...
	// resets modals after.
	// returns a token for continuing a promise
	var destroyFavoriteSuccess = function destroyFavoriteSuccess() {
	  message('Favorite is Updated');
	  resetModalValues();
	  return store.user.token;
	};
	...
	// destroy failure
	var destroyFavoriteFailure = function destroyFavoriteFailure() {
	  alert('Either not your favorite, or field fail.');
	};
	...
	var getUpdateNameListSuccess = function getUpdateNameListSuccess(data) {
	  $('#saved-names').empty();
	  store.favorites = [];
	  store.favorites = data.favorites;
	  favorites = store.favorites;
	...
	var getUpdateNameListFailure = function getUpdateNameListFailure() {
	...
	module.exports = {
	  ...
	  createFavoritesSuccess: createFavoritesSuccess,
	  createFavoritesFailure: createFavoritesFailure,
	  getAdjectivesSuccess: getAdjectivesSuccess,
	  getAdjectivesFailure: getAdjectivesFailure,
	  getNounsSuccess: getNounsSuccess,
	  getNounsFailure: getNounsFailure,
	  updateFavoritesSuccess: updateFavoritesSuccess,
	  updateFavoritesFailure: updateFavoritesFailure,
	  destroyFavoriteFailure: destroyFavoriteFailure,
	  destroyFavoriteSuccess: destroyFavoriteSuccess,
	  getUpdateNameListSuccess: getUpdateNameListSuccess,
	  getUpdateNameListFailure: getUpdateNameListFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "\n      <tr data-id="
	    ...
	    + ">\n\n        <th>  <a class=\"load-edit-btn btn\" data-toggle=\"modal\" data-target=\".bs-edit-delete\" href=\"#\"\n          class=\"list-group-item list-group-item-action\" id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">Edit or Delete</a></th>\n\n        <td >The "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.adjective : stack1), depth0))
	    + " "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.noun : stack1), depth0))
	    + "</td>\n\n        <td>"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.adjective : stack1), depth0))
	    + "</td>\n\n        <td>"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.noun : stack1), depth0))
	  ...
	  return "<div data-id=\"favorites-list\">\n\n\n\n<form >\n\n  <table class=\"table table-inverse\">\n    <thead>\n      <tr>\n        <th>Edit or Delete</th>\n        <th>Name<th/\n        <th>Adjective</th>\n        <th>Noun</th>\n        <th>Comment</th>\n      </tr>\n    </thead>\n    <tbody>\n\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.userFavorites : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "\n    </tbody>\n\n  </table>\n";
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var checkFavoritesArray = function checkFavoritesArray(array) {
	  for (var i = 0; i < array.length; ++i) {
	    if (array[i].user.id === store.user.id) {
	...
	var checkFavorites = function checkFavorites(id) {
	  var favorites = store.favorites;
	...
	var generateName = function generateName(event) {
	  ...
	  // puts them together
	  var currentName = 'The ' + adjective + ' ' + noun;
	  // displays the name
	  nameDisplay(currentName);
	};

	// post favorite function
	var postFavorite = function postFavorite(event) {
	  event.preventDefault();
	...
	var patchFavorite = function patchFavorite(event) {
	  event.preventDefault();
	  var idInitial = parseInt(document.getElementById('id-text').value);
	  var adjective = document.getElementById('adjective-text').value;
	  var noun = document.getElementById('noun-text').value;
	  var comment = document.getElementById('comment-text').value;
	  var idCheckOne = parseInt(idInitial);
	  var idCheckTwo = testID(idCheckOne);
	...
	var deleteFavorite = function deleteFavorite(event) {
	  event.preventDefault();
	  var idInitial = parseInt(document.getElementById('id-text').value);
	  var adjective = document.getElementById('adjective-text').value;
	  var noun = document.getElementById('noun-text').value;
